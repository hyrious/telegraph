...
function render_style(file, outdir, options = {}) {
  const p = esbuild.build({
    entryPoints: [file],
    bundle: true,
    outdir,
...
async function build(root = '.', options = { minify: true }) {
  ...
  for (const file of get_files(src)) {
    ...
    if (file.endsWith('.css')) {
      tasks.push(render_style(file, root, { minify: options.minify }))
  ...
  if (options.minify)
    for (const file in outputs)
      if (file.endsWith('.html') && outputs[file]) {
        outputs[file] = minify(outputs[file], {
  ...
  for (const file in outputs)
    if (!outdated.has(file) && outputs[file] && options.write !== false) {
      tasks.push(write(file, outputs[file]))
    }

  for (const file of outdated) {
    if (file in outputs) {
      if (outputs[file] && options.write !== false)
        tasks.push(
...
async function serve(root = '.') {
  const { env, data, renderer } = await build(root, { minify: false, write: false })
  ...
  server.on('request', (req, res) => {
    ...
    req.once('end', () => {
      ...
      const pad = Math.max(0, process.stdout.columns - 21 - req.url.length)
      console.log(
        '    ' + statusColor(res.statusCode),
        req.method,
        bold(req.url),
        ' '.repeat(pad),
        dim(`${(dur[1] / 1e6).toFixed(2)}ms`),
      )
    })
  })
